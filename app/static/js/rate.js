  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      if ($("#no-avg").length) {
        const avgHtml = `
          <div class="d-flex flex-column align-items-center gap-3">
            <input type="text" class="kv-fa rating-display" 
              ...
              readonly>
            <span class="text-muted fs-5">(${(Math.round(parseFloat(data.avgRating) * 10) / 10).toFixed(1)})</span>
          </div>`
        ...
        $(".text-muted.fs-5").text(`(${(Math.round(parseFloat(data.avgRating) * 10) / 10).toFixed(1)})`)
      }
